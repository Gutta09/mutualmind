const SENTIMENT = {
  ...
  neutral: { borderColor: 'var(--text4)', color: 'var(--text3)', bg: 'rgba(90, 84, 74,.15)', label: '→ Neutral' },
}
...
export function SentimentGauge({ articles = [] }) {
  ...
  return (
    <div style={{ background: 'var(--panel-grad)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, marginBottom: 14 }}>
      <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text4)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 10 }}>Overall Sentiment</p>
      <div style={{ display: 'flex', height: 8, borderRadius: 4, overflow: 'hidden', gap: 1, marginBottom: 8 }}>
        ...
        <div style={{ background: 'var(--text4)', width: `${(counts.neutral / total) * 100}%`, transition: 'width .3s' }} />
        <div style={{ background: '#e7625f', width: `${(counts.bearish / total) * 100}%`, transition: 'width .3s', borderRadius: '0 4px 4px 0' }} />
      </div>
      <div style={{ display: 'flex', gap: 16, fontSize: 12 }}>
        {[['bullish','#7fb069'],['neutral','var(--text3)'],['bearish','#e7625f']].map(([k, c]) => (
...
export default function NewsArticleCard({ article }) {
  ...
  return (
    <div style={{ background: 'var(--panel-grad)', border: '1px solid var(--border)', borderLeft: `3px solid ${s.borderColor}`, borderRadius: 14, padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 8 }}>
        <a href={article.url} target="_blank" rel="noopener noreferrer"
          style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', textDecoration: 'none', lineHeight: 1.45 }}
      ...
      {article.description && (
        <p style={{ fontSize: 12, color: 'var(--text4)', marginBottom: 8, lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {article.description}
        </p>
      )}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, color: 'var(--muted)' }}>
        ...
        {article.sentiment_reason && (
          <span style={{ color: 'var(--text4)', fontStyle: 'italic', textAlign: 'right', maxWidth: 260 }}>{article.sentiment_reason}</span>
        )}
      </div>
    </div>
  )
